...
//    }
    ...
    public User user(@Value("${user.id}") Long id, @Value("${user.name}") String name,
                     ...
                     @Value("${user.wife:${my.user.wife:liuyifei}}") String wifies) {
        User user = new User();
        user.setId(id);
        user.setName(name);
        user.setAge(age);
...
//    @Autowired
    public User user2(
        ...
        Integer age = environment.getProperty("user.age", Integer.class,36);
        String wife = environment.getProperty("user.wife", String.class,
                environment.getProperty("my.user.wife", String.class));
        User user = new User();
        user.setId(id);
        user.setName(name);
        user.setAge(age);
